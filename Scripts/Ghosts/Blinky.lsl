...
vector housePosition = <128.0, 130.0, 21.5>;
vector outOfMazePosition = <128.0, 134.0, 21.5>;
vector blinkyHomeCorner = <148.0, 148.0, 21.5>;
...
UpdateGhostSprite()
{
    ...
    {
        llSetTexture("SleepyBlinky", ALL_SIDES);
    }
    ...
    {
        llSetTexture("AngryBlinky", ALL_SIDES);
        llSetTextureAnim(ANIM_ON, 0, 4, 1, frame, 1.0, 0.0);
    }
    else
    {
...
{
    vector targetPosition = housePosition;
    ...
    else if (llGetListLength(details) > 0)
    {
        if (isScatterMode)
        {
            targetPosition = blinkyHomeCorner;
        }
        else
        {
            targetPosition = llList2Vector(details, 0);
        }
    }
...
default
{
    ...
    timer()
    {
        ...
        if (isEaten)
        {
            if (llVecDist(pos, housePosition) < 0.6)
            {
                RespawnGhost();
            }
        }
        ...
        if ((distX < 0.25 && distY < 0.25) || !aheadClear)
        {
            ...
            if (isEaten)
            {
                nextDir = chooseBlinkyDirection(pos);
            }
            else if (isFrightened)
            {
    ...
    listen(integer channel, string name, key id, string msg)
    {
        ...
        else if (channel == 777)
        {
            ...
            {
                ApplyDistraction( blinkyHomeCorner, 25.0 );
            }
